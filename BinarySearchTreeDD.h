...
*****************************************************/
    ...
    void displayDeep(BinaryNode *t, int k, int node = 0 ) const
    {
        if(t->left != nullptr)
            displayDeep(t->left, k, node+1);
        if(t->right != nullptr)
            displayDeep(t->right, k, node+1);
        if(k == node)
            displayTree(t, node, cout);
    }
