//
//  BinarySearchTreeCD.h
//  Assignment 2 BST
//
//  Created by Varun Ravichandran on 2020-11-01.
//
...
*****************************************************/
    ...
    int countDeep(BinaryNode *t, int k, int node = 0 ) const
    {
        ...
        if(k == node)
        {
            int count_two = 1;
            if(t->left != nullptr)
                count_two += countDeep(t->left, k, node);
            if(t->right != nullptr)
                count_two += countDeep(t->right, k, node);
            count += count_two;
        }
        if(t->left != nullptr)
            count += countDeep(t->left, k, node+1);
    
        if(t->right != nullptr)
            count += countDeep(t->right, k, node+1);
